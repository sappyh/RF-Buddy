...
public:
		  ...
		  d_threshold(threshold)
	{
		...
		//Sample index for the annotation
		d_sample_start = 0;
		d_sample_count = 0;
				...
				while (count < ninput)
				{
					...
					if (d_preamble_cnt == 0)
					{
						...
						if (threshold < d_threshold)
						{
							...
							//Collect the information from where the SFD is starting
					...
					if (d_chip_cnt == 0)
					{
						...
						if (d_packet_byte_index % 2 == 0)
						{
							...
							{ // packet is filled, including CRC. might do check later in here
								...
								meta = pmt::dict_add(meta, pmt::mp("sample_start"), pmt::from_uint64(d_sample_start));
								meta = pmt::dict_add(meta, pmt::mp("sample_count"), pmt::from_uint64(d_sample_count));
